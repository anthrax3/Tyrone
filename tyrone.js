...
client.addListener('pm', function(nick, message) {
  ...
  if (com) {
    ...
    if (config.masters.indexOf(nick) !== -1) {
       ...
       */
      if (com.command == 'add') {
        functions.addPhrase(com.params.join(' '));
        client.say(nick, 'Successfully added phrase');
      }
      
      /**
       * reload phrases.txt
       */
      if (com.command == 'reload') {
        functions.reloadPhrases();
        client.say(nick, 'Successfully reloaded phrases.txt');
      }
